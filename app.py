...
def precipitation():
    ...
    latest_year = session.query(
        Measurement.date).order_by(
        Measurement.date.desc()).first()[0]
    start_year = dt.datetime.strptime(
        latest_year, "%Y-%m-%d") - dt.timedelta(days=365)

    results = session.query(
        Measurement.date, Measurement.prcp).filter(
        Measurement.date >= start_year).all()
...
def start_end(start, end):
    """Temp_MIN, Temp_AVG, and Temp_MAX for dates between start and end.
        enter a `/` between start and end date
    """
